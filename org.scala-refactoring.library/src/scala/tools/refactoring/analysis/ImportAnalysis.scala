...
trait ImportAnalysis extends TreeFactory with EnrichedTrees with TreeTransformations with CompilerAccess {
   ...
   */
  sealed trait ImportTree extends Iterable[ImportTree] {
    /**
     ...
     */
    /*
    override def foreach[U](f: ImportTree => U) = {
      f(this)
      children.foreach(_.foreach(f))
    }
    */
    
    def iterator: Iterator[ImportTree] = {
      new MyIterator(this, children)
    }
    
    //TODO: Provide correct implementation
    class MyIterator(val head: ImportTree, tail: Seq[ImportTree]) extends AbstractIterator[ImportTree] {
      var first = false;
      def hasNext: Boolean = {
        if (first){
          head != null
